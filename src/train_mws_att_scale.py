...
from model_linear_att_scale import GPTLinear
...
def train_step(
    ...
    att_ablate_mask = torch.ones(size=(train_data.shape[0], 1, 2*num_tokens, 2*num_tokens)).to(device)
    
    if step >= config.train.att_jump_step:
        # Do not ablate the partial solution position that is already learnt fast
        # att_ablate_mask[:, :, num_tokens, 0] = config.model.att_ablate_c

        for i in range(num_tokens - 1):
            att_ablate_mask[:, :, num_tokens + i + 1, i : i + 2] = config.train.att_scale_c

    model.train()
    optim.zero_grad(set_to_none=True)

    _, _, _, loss = model(
        train_data[:, :-1], targets=train_data[:, 1:], att_ablate_mask=att_ablate_mask
    )
    loss.backward()
    ...
    with torch.no_grad():
        ...
        attn_map, pre_lm_h, _, train_loss = model(train_data[:, :-1], targets=train_data[:, 1:], att_ablate_mask=att_ablate_mask)
        
        train_pred = model.generate(
            idx=train_data[:, :prompt_len],
            max_new_tokens=gen_len,
            att_ablate_mask=att_ablate_mask
        )
        test_pred = model.generate(
            idx=test_data[:, :prompt_len],
            max_new_tokens=gen_len,
            att_ablate_mask=att_ablate_mask[:n_test]
        )
