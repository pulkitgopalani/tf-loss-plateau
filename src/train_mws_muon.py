...
from model_linear_init_scale import GPTLinearInitScale
...
if __name__ == "__main__":
    ...
    model = GPTLinearInitScale(config.model, return_att=True).to(device)
    
    hidden_weights = [p for p in model.transformer.h[0].parameters() if p.ndim >= 2]
    hidden_gains_biases = [p for p in model.transformer.h[0].parameters() if p.ndim < 2]
    nonhidden_params = [
    ...
    param_groups = [
        dict(
            ...
            use_muon=True,
            lr=config.train.muon_lr,
            weight_decay=config.train.muon_wd,
        ),
        dict(
            params=hidden_gains_biases + nonhidden_params,
            use_muon=False,
            lr=config.train.adamw_lr,
            weight_decay=config.train.adamw_wd,
        ),
    ]
    optim = SingleDeviceMuonWithAuxAdam(param_groups)

    if config.train.wandb:
        wandb_run_name = f"mws_muon_init_{config.model.init_scale}"
        wandb.login(key="")
        wandb.init(project="muon-analysis", name=wandb_run_name, config=config)
        wandb.watch(model)
