...
def train_step(
    ...
    prompt_len = 2 * num_tokens + 2
    gen_len = num_tokens
    acc_start = 2 * num_tokens + 2
    ...
    with torch.no_grad():
        ...
        predicted_counts = batch_count(test_pred[:, acc_start:], max_len=config.data.max_num + 1)[:, config.data.min_num :]
        true_counts = batch_count(test_data[:, acc_start:], max_len=config.data.max_num + 1)[:, config.data.min_num :]

        count_check = torch.mean(
            (predicted_counts == true_counts).to(float)
        )
        ...
        # attn_map_output_seq1 = attn_map1[:, :, acc_start-1:, num_tokens + 1: 2 * num_tokens + 1]
        att_mask1 = torch.zeros_like(attn_map_output_seq1).to(device)

        for i in range(num_tokens):
            att_mask1[:, :, i, num_tokens + 1 + i] = 1
    
        # att_prog_measure1 = torch.sum(torch.abs(attn_map_output_seq1) * att_mask1) / torch.sum(
        #     torch.abs(attn_map_output_seq1)
        # )
        ...
        # Log attention progress measure 
        attn_map_output_seq2 = attn_map2[:, :, acc_start-1:, :]
        # attn_map_output_seq2 = attn_map2[:, :, acc_start-1:, :num_tokens]
        att_mask2 = torch.zeros_like(attn_map_output_seq2).to(device)
        pi = train_data[:, num_tokens + 1 : 2 * num_tokens + 1]
        for data_idx in range(n_train):
            for i in range(num_tokens):
                att_mask2[data_idx, :, i, pi[data_idx, i]-1] = 1

        # att_prog_measure2 = torch.sum(torch.abs(attn_map_output_seq2) * att_mask2) / torch.sum(
        #     torch.abs(attn_map_output_seq2)
        # )
        att_prog_measure2 = torch.mean(
            torch.sum(torch.abs(attn_map_output_seq2) * att_mask2, dim=(-3, -2, -1)) / 
            torch.sum(torch.abs(attn_map_output_seq2), dim=(-3, -2, -1)),
            dim=0
        )

        # Alternative strategy progress measure
        alt_attn_map_output_seq1 = attn_map1[:, :, num_tokens + 1: 2 * num_tokens + 1, :]
        alt_att_mask1 = att_mask2.clone()
        alt_att_prog_measure1 = torch.mean(
            torch.sum(torch.abs(alt_attn_map_output_seq1) * alt_att_mask1, dim=(-3, -2, -1)) / 
            torch.sum(torch.abs(alt_attn_map_output_seq1), dim=(-3, -2, -1)),
            dim=0
        )
        # alt_att_prog_measure1 = torch.sum(torch.abs(alt_attn_map_output_seq1) * alt_att_mask1) / torch.sum(
        #     torch.abs(alt_attn_map_output_seq1)
        # )

        alt_attn_map_output_seq2 = attn_map2[:, :, acc_start-1:, :]
        alt_att_mask2 = att_mask1.clone()
        alt_att_prog_measure2 = torch.mean(
            torch.sum(torch.abs(alt_attn_map_output_seq2) * alt_att_mask2, dim=(-3, -2, -1)) / 
            torch.sum(torch.abs(alt_attn_map_output_seq2), dim=(-3, -2, -1)),
            dim=0
        )
        # alt_att_prog_measure2 = torch.sum(torch.abs(alt_attn_map_output_seq2) * alt_att_mask2) / torch.sum(
        #     torch.abs(alt_attn_map_output_seq2)
        # )
        ...
        print(f"data: {test_data[0]}")
        print(f"predict: {test_pred[0]}")
        ...
        if config.train.wandb:
            log_data = {
                ...
                "train acc": train_acc,
                "test_acc": test_acc,
                "att_progress_measure1": att_prog_measure1,
                "att_progress_measure2": att_prog_measure2,
                "alt_att_progress_measure1": alt_att_prog_measure1,
                "alt_att_progress_measure2": alt_att_prog_measure2,
...
if __name__ == "__main__":
    ...
    data_sampler = Permute(
        min_num=config.data.min_num,
        max_num=config.data.max_num,
        compose=config.data.compose,
    )

    config.model.vocab_size = max(config.data.max_num, config.data.num_tokens) + 1
    config.model.block_size = 3 * config.data.num_tokens + 2
