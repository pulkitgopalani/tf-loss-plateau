...
if __name__ == "__main__":
    ...
    optimal_state_dict = torch.load("./path-to-weights")['model']
    # initial_state_dict = torch.load("./weights_final/mws_k2_l1_h1_a16_n16_0.tar")['model']

    # Replace parameter by optimal parameters for attention map
    for k in model_state_dict.keys():
        # if ('attn.q' in k) or ('attn.k' in k) or ('wte' in k) or ('wpe' in k) or ('ln_1' in k):
        # if ('attn' in k) or ('wte' in k) or ('wpe' in k) or ('ln_1' in k):
        if ('mlp' in k) or ('ln_2' in k) or ('lm_head' in k) or ('ln_f' in k):
        # if ('wte' in k) or ('wpe' in k) or ('lm_head' in k) or ('ln_f' in k):
            model_state_dict[k] = optimal_state_dict[k].clone().detach()
    
    model.load_state_dict(model_state_dict)

    # Freeze optimal parameters for attention map
    for name, param in model.named_parameters():
        # if ('attn.q' in name) or ('attn.k' in name) or ('wte' in name) or ('wpe' in name) or ('ln_1' in name):
        # if ('attn' in name) or ('wte' in name) or ('wpe' in name) or ('ln_1' in name):
        if ('mlp' in name) or ('ln_2' in name) or ('lm_head' in name) or ('ln_f' in name):
        # if ('wte' in name) or ('wpe' in name) or ('lm_head' in name) or ('ln_f' in name):
            param.requires_grad = False
            print(f"Frozen {name} at optimal")
