...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, att_ablate_mask=None):
        B, T, C = (
            x.size()
        ...
        if att_ablate_mask is not None:
            att = att * att_ablate_mask
...
class Block(nn.Module):
    ...
    def forward(self, x, att_ablate_mask=None):
        if self.return_att:
            x_prev, att = self.attn(self.ln_1(x), att_ablate_mask=att_ablate_mask)
            x = x + x_prev
...
class GPTLinear(nn.Module):
    ...
    def forward(self, idx, targets=None, att_ablate_mask=None):
        device = idx.device
        b, t = idx.size()
        assert (
            t <= self.block_size
        ...
        for block in self.transformer.h:
            if self.return_att:
                x, att = block(x, att_ablate_mask=att_ablate_mask)
    ...
    @torch.no_grad()
    def generate(
        self, idx, max_new_tokens, att_ablate_mask=None, temperature=1.0, do_sample=False, top_k=None
    ):
        """
        ...
        for _ in range(max_new_tokens):
            ...
            prompt_len = idx_cond.shape[-1]
            _, _, logits, _ = self(idx_cond, att_ablate_mask=att_ablate_mask[:, :, :prompt_len, :prompt_len])
