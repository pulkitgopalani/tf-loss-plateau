...
def train_step(
    ...
    with torch.no_grad():
        ...
        if config.data.name == "reverse":
            for i in range(num_tokens):
                att_mask[:, :, i, num_tokens - i - 1] = 1
        elif config.data.name == "copy":
            for i in range(num_tokens):
                att_mask[:, :, i, i] = 1
        ...
        if config.train.wandb:
            ...
            log_data = {
                ...
                "mean_cosine_sim": torch.sum(logit_cs) / (0.5 * (num_tokens) * (num_tokens-1))
            }
...
if __name__ == "__main__":
    ...
    if config.data.name == "copy":
        config.model.vocab_size = config.data.max_num + 1
    ...
    elif config.data.name == "reverse":
        config.model.vocab_size = config.data.max_num + 1
        config.model.block_size = 2 * config.data.num_tokens + 1
        data_sampler = Reverse(
            min_num=config.data.min_num,
            max_num=config.data.max_num,
        )
